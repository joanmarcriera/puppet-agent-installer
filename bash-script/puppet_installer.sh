...
setUpInstallPkg()
{
	...
	[ -n "${DistroBasedOn+x}" ] && tell  "shootProfile has  been run correctly. We are on setUPInstallPkg" || die "neeed shootprofile"
	  ...
	  echo -n "Installing $i on    $DistroBasedOn      ."
				...
				case ${REVISION} in 
				6)
					DEBIANRELEASE=squeeze
					die $DEBIANRELEASE
					;;	
				7)
					DEBIANRELEASE=wheezy
					die $DEBIANRELEASE
					;;	
				8)
				...
				*)
				...
				DEBIANPACKAGE=puppetlabs-release-${DEBIANRELEASE}.deb
				wget https://apt.puppetlabs.com/$DEBIANPACKAGE
				dpkg -i ${DEBIANPACKAGE}
				apt-get update
